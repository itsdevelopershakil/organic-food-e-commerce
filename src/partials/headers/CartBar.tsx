    ...
    >
      ...
      >
        ...
        <div className="w-full overflow-y-auto scrollbar-hidden">
          <div className="pt-3">
            {cartItems.length === 0 && (
              <p className="mt-5 md:text-lg">No cart items added.</p>
            )}
            {cartItems.length > 0 &&
              cartItems.map(
                ({ image, price, quantity, title }: DataObject, i: number) => (
                  <div key={i}>
        ...
        <div className="mt-auto w-full">
          ...
          <div>
            <Link
              ...
              } rounded-[43px] py-2 md:py-4 text-sm md:text-base text-center text-white font-semibold mb-3`}
              to="/checkout"
            >
              Checkout
            </Link>
